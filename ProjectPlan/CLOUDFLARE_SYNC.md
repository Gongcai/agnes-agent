...
> 状态：设计完成，Phase 0、Phase 1 与 Phase 2 已完成
> 调研日期：2026-07-16  
...
当前 Session payload 为避免泄漏路径而整体排除了 `workspace_id`，但 workspace folder binding 已拆到独立本地表。Phase 3 实现 pull/merge 时需修正为：同步逻辑 `workspace_id`，继续排除 `workspace_bindings.folder_path / last_validated_at`。这是主界面跨设备显示“工作区会话”的前置条件。
