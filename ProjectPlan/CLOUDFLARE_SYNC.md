...
当前 `workspaces.folder_path` 同时表达逻辑工作区和设备路径，不适合多设备。同步前应拆分概念：
...
CREATE INDEX idx_sync_entities_bootstrap
  ON sync_entities(owner_id, entity_type, latest_server_seq);
```
...
| 409 | `REVISION_CONFLICT` | baseRevision 已过期 | 拉取远端版本并进入合并 |
...
- 将 USER.md / MEMORY.md 建模为明确同步实体；
- 拆分 workspace 逻辑信息与设备 folder binding；
- 为所有同步实体补齐 tombstone、本地 version 和稳定 UUID；
...
- `settings` 已分类为 `syncable / device-local / secret / unknown`。通用 renderer `get_setting/set_setting` 仅允许访问 `ui:*`；模型角色和显式记忆继续走专用命令。
...
- 该投影器目前是同步协议的数据边界基础，还未接入事务性 outbox。Phase 0 剩余工作完成后再由业务仓库在同一事务内生成不可变 payload。

### Phase 1：Worker / D1 骨架

- 建 `workers/sync-api` workspace；
- 建本地 D1 migrations 和 schema；
- 实现 auth、health、push、pull、bootstrap；
- 建 production D1，按需建 staging；
- 添加协议 fixtures 和 Worker 集成测试；
- 使用假数据验证 rows read/written 和 Worker CPU。
