...
> 状态：Phase 0-4 已完成，E2EE Worker 已部署且 production D1 为空
> 调研日期：2026-07-17
...
- **R2**：存放客户端压缩并加密后的源文件、备份和向量制品等大对象；桌面端通过 `r2-managed` 账户复用同步凭证。
...
| R2 | 头像、附件、源文件、加密导出和向量制品 | 已接入数据面 | 大对象不应进入 D1 变更行；客户端先 E2EE |
...
`SYNC_DEVICE_IDENTITIES` 保存首台设备的 SHA-256 指纹与身份映射，作为管理员 bootstrap；配对产生的
后续设备只把唯一 SHA-256 fingerprint 写入 D1 `devices.credential_fingerprint`。owner 和设备身份始终
由服务端映射，请求体不能指定 owner，业务请求的 deviceId 必须与凭证映射交叉校验。

Phase 4D 已用 SPAKE2 一次性配对完成自助注册：旧设备认证授权、新设备获得独立令牌、Worker 不接触
令牌明文或 keyset。会话限制、过期清理、撤销联动见 `E2EE.md`；长期设备凭证的显式自助轮换/过期和
网关级 IP 限速仍可在公网多用户部署前增强，不影响当前个人单 owner 的高熵令牌安全边界。本地
`AUTH_MODE=test` 只用于未提交的开发环境。E2EE 已完成并部署，本轮仍未上传真实业务数据。
...
#### Phase 4D 安全配对、轮换与恢复演练记录（2026-07-17）

- 新增 RustCrypto `spake2 0.4`，旧设备/新设备分别使用 A/B 角色和绑定 session UUID 的 identity。
  一次性配对码包含随机 256-bit secret；SPAKE2 结果经 HKDF 域隔离后，分别用不同 AAD 的
  XChaCha20-Poly1305 proof 与 transfer bundle 完成显式密钥确认和 keyset/设备令牌传递，不自行拼装
  ECDH。详细格式和威胁模型见 `ProjectPlan/E2EE.md`。
- Worker 新增 10 分钟 `pairing_sessions` 状态机：认证旧设备 create/inspect/finalize，公开新设备
  get/join/package，认证新设备 consume。D1 只保存 PAKE 消息、设备元数据、Bearer SHA-256 指纹和
  不透明密文包；同一 join/finalize 可安全重试，consume 清空密文，Cron 每小时清理过期会话。
- Bearer 认证在保留 `SYNC_DEVICE_IDENTITIES` 管理员 bootstrap 的同时，可按 D1 唯一 credential
  fingerprint 解析配对产生的设备；撤销仍在授权层阻断设备，并同步删除该设备关联的配对会话。
- 新设备在 Rust 内原子安装独立 Bearer credential 和完整 keyset，逐项读回 Keyring 验证后才推进
  SQLite confirmed version；配对码离开设置页或完成后清空，master key/keyset/token 从不进入
  Renderer IPC。
- E2EE 轮换使用两阶段门禁：先向 Keyring 追加新 active key 并导出新 Recovery Bundle，confirmed
  version 未推进时暂停所有业务同步；用户确认后新写入使用 vNext，读取继续按 envelope version
  选择旧 key。已配置设备只接受旧 key 完全相同的单调 keyset 超集，拒绝分叉、删减与降级。
- 自动恢复演练使用两套隔离 DB/Keyring：target 先恢复 v1，再导入 source 轮换后的 v2 bundle，随后
  同时成功解密 v1 与 v2 payload。安全处置明确为“撤销疑似失控设备后立即轮换”；这只保护新数据，
  不对已泄露旧 key/旧数据提供追溯保密。
- 最终本地验证为 Rust 106 项通过、1 项显式远端凭证测试忽略；Worker 17 项通过且 typecheck 成功；
  前端 4 项通过且 production build 成功。普通 Clippy 通过，严格告警基线见本轮提交说明。
- production D1 migration `0003_secure_pairing.sql` 已应用，Worker 版本
  `7316feb3-48b1-4635-8363-a83e78e7dc33` 已部署并启用 `17 * * * *` 清理任务。部署前四表为 0，部署后
  `sync_entities / sync_changes / sync_acks / devices / pairing_sessions` 五表均为 0。
- 线上实时 tail 用无凭证 health 和固定假数据配对 POST 审计：只出现 method、脱敏 URL 和 outcome，
  未出现 Authorization、请求 body、配对 proof 或 transfer bundle。本轮没有上传真实业务数据。

#### Phase 4E Planner E2EE 同步记录（2026-07-18）

- `calendar`、`calendar_event`、`event_exception`、`task_list`、`task` 已加入同一 E2EE
  白名单、事务性 outbox、墓碑、CAS 和 pull/bootstrap 协议；`notifications` 明确保留设备本地，绝不进入
  payload。
- 本地 Planner 写入会与不可变 outbox 投影在一个 SQLite `BEGIN IMMEDIATE` 事务内提交。远端落地走专用
  DB 路径，不生成 outbox，因此不会形成同步回声。
- 日历系列与单次例外独立同步：替换事件先于例外出队，例外取消、恢复、系列规则变更及系列删除均以墓碑传播；
  同一例外的并发“取消/替换”保留为显式冲突，不静默覆盖。
- 待办列表先于待办 bootstrap。加密 payload 无法用于服务端拓扑排序，因此本地临时保存尚未到达父项的子任务引用，
  父项到达后自动补回。重复待办下一实例的 ID 由当前实例和下一截止时间稳定派生，避免双设备同时完成时出现重复实例。
- 日历事件、列表和待办的非重叠字段使用现有三方合并；同字段、删除与例外状态冲突进入设置页同步冲突入口。设置页已提供
  Planner 实体中文标签。
- Worker bootstrap 顺序扩展到 Planner 依赖层级；`0004_planner_bootstrap_order.sql` 将更新已部署 D1 的
  expression index。回归覆盖本地 outbox、远端无回声、例外墓碑、乱序父子待办恢复及 Worker 排序。
...
- [x] 按 `STORAGE_AND_RAG.md` 接入 R2 加密制品：Worker Multipart 上传、分片 hash、完成校验、owner-scoped opaque key、manifest/replica/change/device state API 和 Range 下载；
- [x] 桌面端自动维护 `r2-managed` 账户，只读复用同步 Keyring 凭证，并兼容 Bearer 与 Cloudflare Access 鉴权；
- [x] Rust 客户端接入 object manifest/change/device-state API，并将 object cursor 与聊天 pull cursor 分离；
- [x] 桌面端接入独立 artifact replication coordinator：动态读取 keyset/同步凭证，后台有界处理 R2 ready replica，验证安装成功后再推进 object cursor；
- R2 对象也必须遵循认证、owner 隔离和客户端加密。

### Phase 5A：真实桌面同步验收（2026-07-21）

- Wrangler OAuth、`SYNC_DEVICE_IDENTITIES`、`agnes-sync` D1 和 `agnes-blobs` R2 bucket 均已配置；远端 D1 migration 无待应用项，Worker 最新部署包含 R2 binding。
- 桌面端完成 E2EE 恢复材料确认、设备 token 写入 Keyring 和真实 Worker Bearer 认证；547 条测试同步变更可在一次手动同步中自动排空，冲突/失败保持为 0/0。
- 远端当前仅保留测试数据（1 台测试设备、215 个测试实体、547 条同步变更）；一个测试知识制品已完成 R2 Multipart 上传，manifest/replica/change/device state 各写入一条记录，重复发布复用同一 artifact。
- 远端直接下载所得密文对象为 59,271 字节，SHA-256 与 D1 manifest 一致；Wrangler bucket 汇总计数当时仍为 0，属于聚合统计延迟，实际对象读取成功。
- 真实 R2 桌面发布验收已完成；安卓端多设备 Range 下载、原子安装和设备状态回写继续留到 Android 客户端完成后统一执行。
