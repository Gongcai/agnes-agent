...
> 状态：设计完成，Phase 0、Phase 1 与 Phase 2A 已完成
> 调研日期：2026-07-16  
...
- [ ] 接入 Cloudflare Access 身份映射与 OS Keyring 凭证，完成远端假数据端到端验证。
...
Phase 2B 必须先让 Worker 将经 Cloudflare Access 验证的 service token 映射为固定 `ownerId/deviceId`；仅在 Access 前置放行请求不足以替代 Worker 的设备级授权。完成该映射前保持远端 `AUTH_MODE=disabled`。
