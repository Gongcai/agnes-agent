...
| V0.3 | Cloudflare Workers + D1 + 事务性 outbox + 增量同步 + E2EE | 进行中：Phase 0 数据边界和 Phase 1 Worker/D1 已完成；远端 APAC D1、Worker 部署、migration、幂等 push/pull/bootstrap/ack 假数据验证与清理均已通过，当前远端认证关闭；下一步进入 Phase 2 Rust 事务性 outbox |
