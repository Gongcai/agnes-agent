...
| V0.5 | MCP + diff review + workspace sandbox + tool audit + 多模型 fallback | 进行中：工具、审批、Linux 沙箱、审计、模型路由、内置 `web_search/web_fetch`、可配置 SearXNG/Brave Search Provider、MCP Client 与主模型零输出故障回退已完成；搜索与模型回退均使用显式顺序和归一化失败类别。浏览器操作与 diff review 待后续补齐 |
