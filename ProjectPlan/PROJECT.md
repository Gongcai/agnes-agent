...
| V0.5 | MCP + diff review + workspace sandbox + tool audit + 多模型 fallback | 进行中：工具、审批、Linux 沙箱、审计、模型路由、内置 `web_search/web_fetch`、MCP Client 与主模型零输出故障回退已完成；回退按有序备用模型执行并显示实际模型与失败类别。正式搜索 API Provider、浏览器操作与 diff review 待后续补齐 |
