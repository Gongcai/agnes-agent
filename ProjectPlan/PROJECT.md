...
- 网盘通过 `ObjectStorageProvider` 端口接入；Google Drive 使用官方 API，夸克通过可替换的 `quarkpan` 等社区逆向 adapter 接入并要求用户显式启用，二者是只读浏览器完成后的下一项高优先级工作。R2 继续作为应用托管的加密对象副本。
...
| V0.6 | 侧边栏子功能导航 + 知识库 + 本地 RAG + 加密向量制品 + 通用网盘 Provider | 进行中：侧边栏、知识库 schema/迁移/权限、文本导入分块、FTS5 + sqlite-vec、RRF 检索和安全上下文注入已完成；通用网盘窄端口、开放注册表、逻辑账户/本机 binding 分层、Keyring 凭证端口、传输队列、应用服务、IPC 和隐藏工作区已完成。下一步接入 Google Drive 官方 adapter 与夸克社区 adapter，首个可连接后开放网盘入口；加密制品与 R2 随后接入 |
