...
| V0.3 | Cloudflare Workers + D1 + 事务性 outbox + 增量同步 + E2EE | 进行中：Phase 0 数据边界、Phase 1 Worker/D1 和 Phase 2A Agent/Session 事务性 outbox + push 引擎已完成；具备持久化 Device UUID/HLC、断线重试、单飞调度、手动同步和状态展示。当前远端仍关闭认证，下一步接入 Cloudflare Access 身份映射并做远端假数据端到端验证 |
