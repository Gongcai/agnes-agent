...
| V0.3 | Cloudflare Workers + D1 + 事务性 outbox + 增量同步 + E2EE | 进行中：Phase 0-2、3A、3B 已完成；Agent/Workspace/Session、完成态 Message、USER.md/MEMORY.md 和结构化记忆均具备事务性 push/pull/bootstrap，向量与私有消息片段保持本地。下一步进入 Phase 3C：字段级三方合并、冲突/设备管理 UI，然后进入 E2EE |
