...
| V0.6 | 侧边栏子功能导航 + 知识库 + 本地 RAG + 加密向量制品 + 通用网盘 Provider | 进行中：本地 RAG 与通用网盘 Provider 基础已完成；Google Drive 官方 adapter 已接入 Desktop OAuth + PKCE、Keyring token 轮换、文件/配额浏览、文件与目录下载、当前目录断点上传、Workspace 文档导出、Range 下载、`appDataFolder` 对象上传及用户文件移入回收站；夸克 community adapter 已接入文本/JSON Cookie、二维码授权、目录/配额、下载、Range 下载、分片上传及文件移入回收站；Google/夸克文件已可直接导入知识库和书架，统一契约测试覆盖授权丢失、字段变化、限流、断点恢复和 Provider 切换。真实账户验收待用户凭证；下一步接入加密制品 manifest 与 R2 |
