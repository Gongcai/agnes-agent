...
| V0.6 | 侧边栏子功能导航 + 知识库 + 本地 RAG + 加密向量制品 + 通用网盘 Provider | 进行中：本地 RAG、artifact v1 加密/校验/原子安装、SQLite manifest/replica/device 状态、Google Drive 对象副本编排、R2 Worker Multipart/Range 数据面与 Rust object 控制面客户端已完成；聊天 pull cursor 与 object cursor 已分离。Worker 契约测试覆盖分片校验、owner 隔离、逻辑版本 CAS、幂等重放、设备落地状态和过期清理。真实 Google 账户、R2 bucket 和端到端多设备验收仍待部署凭证；本地磁盘配额/GC、R2 孤儿对账及知识库页面设备覆盖状态待后续补齐 |
