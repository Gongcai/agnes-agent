...
原有 `ApprovalTier` 保留在角色策略中供配置兼容与审计快照使用；当前人工审批入口以会话权限模式为准。

| 模式 | 当前行为 |
|---|---|
| `ask_for_approval` | 每次工具调用都请求用户批准 |
| `auto` | 信任当前会话/角色/主模型发起的 Low/Medium 调用并自动执行；High 风险调用仍请求用户二次确认 |
...
Auto 不额外调用一个“审批模型”：当前主模型通过是否发起工具调用表达执行意图，以免为每次普通调用增加一次模型成本和延迟。
模型来源优先级为会话覆盖 → 角色默认 → 全局主模型。High 风险调用无论由哪个模型发起，都必须向用户发送二次确认卡片。

## 十三、文件变更预览（2026-07-21）

`file_write`、`file_edit` 和 `apply_patch` 在物理执行前生成只读变更预览，并随
`agent://tool_call_pending` 或 `agent://tool_call_started` 事件发送给聊天工具卡。需要审批时，用户先查看
diff，再批准或拒绝整次工具调用；预览本身不写文件，也不改变既有权限模式、Tool Policy 或审计链路。

- `file_write` 对比目标文件当前内容与完整新内容；新文件以空内容为基线。
- `file_edit` 按实际唯一替换或 `replace_all` 语义计算结果，不满足执行前置条件时不伪造预览。
- `apply_patch` 直接展示已受 2 MiB 工具参数上限约束的 Codex patch；UI 预览统一截断到 32,000 字符。
- 文件读取只允许当前 workspace 或角色文件策略已授权的路径；文件能力关闭时不读取预览。
- 预览失败不替代执行端校验，也不阻断后续审批。最终写入仍由对应工具重新执行路径、沙箱和内容校验。

当前版本按整次工具调用批准或拒绝，不提供逐 hunk 选择。Shell、Git 和 MCP 可能产生的任意文件副作用无法在
执行前可靠预测，因此不生成虚假的变更预览；它们继续依赖风险审批、沙箱与执行后审计。
