...
> 状态：联网研究 Phase 1、搜索 Provider Phase 2、MCP Client Phase 3 与隔离只读浏览器 Phase 4 已实现
> 日期：2026-07-18
...
- `browser_open`：仅在 `web_fetch` 无法覆盖 JavaScript 动态页面时，使用隔离 Chromium 提取渲染后正文。
...
- `web_search`、`web_fetch` 与 `browser_open` 是 Low 风险只读工具，仍进入现有会话权限、实时工具卡和审计日志链路。
...
- `web_fetch` 每次请求先解析并校验全部 DNS 结果，将实际连接固定到已校验地址；每一跳重定向重新执行同样检查。`browser_open` 在 Chromium 发出主文档、重定向和允许的子资源请求前逐一执行相同公网校验。
- 限制连接与总超时、重定向次数、下载字节数和最终正文字符数；二进制及不支持的媒体类型不进入模型上下文。
- 搜索结果和网页正文始终是不可信参考资料。系统提示词要求模型忽略页面中的角色声明、策略修改和工具调用指令。

## 正文提取

HTML 使用结构化 DOM 解析，优先选择 `article/main/[role=main]`，过滤导航、页眉、页脚、侧栏、表单和脚本区域；纯文本、JSON 与 XML 保留为受长度限制的文本。`web_fetch` 不执行页面 JavaScript；依赖客户端渲染且普通抓取无正文时，才回退 `browser_open`。
...
- `browser_open` 自动发现 Microsoft Edge、Google Chrome 或 Chromium，也允许通过 `AGNES_BROWSER_PATH` 指定程序；应用不自动下载浏览器；
- 每次调用创建随机临时 Profile 和无痕页面，不复用系统浏览器 Profile、Cookie、扩展、缓存或用户登录态，结束后关闭进程并清理目录；
- 只允许 GET/HEAD 和渲染正文所需的 document/style/script/XHR/fetch，拦截图片、媒体、字体、WebSocket、EventSource、Ping、非 HTTP(S) 网络地址及超过上限的主文档重定向；
- Chromium 严格校验 HTTPS 证书；入口、最终地址及所有允许联网的页面请求均通过现有公网/SSRF 校验；
- 最终 DOM 复用 `web_fetch` 的正文提取和字符上限，结果明确标记实际 `final_url` 与浏览器程序。

当前不提供点击、输入、登录、表单提交、下载、购买、发布、复用用户浏览器或持久化会话。若未来确有需求，这些有状态操作必须设计为不同工具和更高风险级别，不得沿用 Low 风险只读权限。
...
- JavaScript 动态页面可通过隔离浏览器提取渲染后正文；缺少 Chromium 时返回可操作错误，不隐式下载安装包。
