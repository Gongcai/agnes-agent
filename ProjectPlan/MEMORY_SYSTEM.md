...
| 结构化记忆库 | SQLite `memory_store` | 可增长、可检索、可管理的事实集合 | AI 按需调用 `memory_search` |
...
比较不区分 ASCII 大小写。结果返回完整的名称、关键词、创建时间、内容和创建人，
默认最多 10 条。关键词只用于增强召回，不替代内容搜索。
