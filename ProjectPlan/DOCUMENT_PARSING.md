...
`document-parserd` 只在标准输出返回一个 JSON 对象：
...
Rust 必须校验协议版本、哈希、大小、解析器指纹、chunk 内容、页码和 metadata 类型。非零退出时 stdout 只返回稳定的用户可见错误对象。
...
后续还应增加格式级限制：PPTX 最大幻灯片数、XLSX 最大工作表和单元格数，以及用户可见的解析进度与取消按钮。
